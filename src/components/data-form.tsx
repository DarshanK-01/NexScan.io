...
export function DataForm({ initialData, isLoading, onSave, sheetActive }: DataFormProps) {
  ...
  useEffect(() => {
    if (initialData) {
      ...
      // Clear form when there is no data
      form.reset({
        branch: '',
        bccdName: '',
        productDescription: '',
  ...
  function onSubmit(values: z.infer<typeof formSchema>) {
    // The 'others' text is intentionally not included in the 'values' object
    // as it's not part of the form schema and should be discarded.
    onSave(values); 
  }
  ...
  const handleTranslate = async () => {
    ...
    const dataToTranslate = { ...currentData, others: otherText };
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder="e.g., 24-inch Monitor" {...field} value={field.value ?? ''} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <FormField
                    ...
                    <FormItem>
                        ...
                        <FormControl>
                        <Input placeholder="e.g., SP-A5432" {...field} value={field.value ?? ''} />
                        </FormControl>
                        <FormMessage />
                    </FormItem>
                    )}
